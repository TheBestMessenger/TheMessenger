...
const DataInput = (props) => {
    ...
    if ((props.info !== 'Password') && (props.info !== 'Password Check') && (props.error === '')) {
        clas = 'sign-input';
        return (
            <>
                <div className={clas}>
        ...
        return (
            <>
                <div className={clas}>
                    <input type={eyeUrl ? 'password' : 'text'} id={props.info} required />
                    <label className='sign-label'>{props.info}</label>
                    <img className='show-button' src={eyeUrl ? 'eye.png' : 'not-eye.png'}
                    onClick={() => setEyeUrl(prev => !prev)} alt='logo' />
                </div>
            </>
        ) 
    }
